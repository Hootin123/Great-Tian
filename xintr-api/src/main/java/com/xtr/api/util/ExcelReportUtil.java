...
//          adjustColumnSize(sheets, sheetNum, fieldNames); // 自动调整列宽
     ...
     */
    @SuppressWarnings({"unchecked"})
    public static void export2Excel2(CustomerPayrollExportDto[] customerPayrollExportDtos, PayCycleBean payCycleBean,
                                     List<CustomerPayrollDto> list, ExportSetInfo exportSetInfo) throws IOException, IllegalAccessException, NoSuchMethodException, InvocationTargetException {
        if (list != null && !list.isEmpty()) {
            //初始化
            init();

            HSSFSheet[] sheets = getSheets(1, new String[]{"【" + payCycleBean.getCurrentPayCycle() + "】工资单"});
            int sheetNum = 0;
            // 标题行
            createTableTitleRow(exportSetInfo, sheets, sheetNum);
            // 日期行
//            createTableDateRow(exportSetInfo, sheets, sheetNum);
            // 表头
            creatTableHeadRow(exportSetInfo, sheets, sheetNum);
            int rowNum = 2;
            for (int i = 0; i < list.size(); i++) {
                CustomerPayrollDto customerPayrollDto = list.get(i);
                //津贴
                ...
                HSSFRow contentRow = sheets[sheetNum].createRow(rowNum);
                contentRow.setHeight((short) 300);
                HSSFCell[] cells = getCells(contentRow, customerPayrollExportDtos.length);
                int cellNum = 1;                    // 去掉一列序号，因此从1开始
                for (int num = 0; num < customerPayrollExportDtos.length; num++) {
                    Object value = null;
                    CustomerPayrollExportDto customerPayrollExportDto = customerPayrollExportDtos[num];
                    //类型 0:基本工资 1:津贴 2:奖金
                    String key = customerPayrollExportDto.getKey();
                    if (customerPayrollExportDto.getType() == 0) {
                        if (key.equals("entryDate")) {
                            value = DateUtil.date2String(customerPayrollDto.getEntryDate(), DateUtil.DATEYEARFORMATTER);
                        } else if (key.equals("addWages")) {
                            value = customerPayrollDto.getAddWages() == null || customerPayrollDto.getAddWages().doubleValue() == 0 ? "-" : customerPayrollDto.getAddWages();
                        } else {
                            value = BeanUtils.getProperty(customerPayrollDto, key);
                            if (key.equals("employMethod")) {//聘用形式
                                if (StringUtils.equals(value.toString(), "1")) {
                                    value = "正式";
                                } else if (StringUtils.equals(value.toString(), "2")) {
                                    value = "劳务";
                                }
                            }
                        }
                        ...
                        if (allowanceList != null && !allowanceList.isEmpty()) {
                            for (int j = 0; j < allowanceList.size(); j++) {
                                if (StringUtils.equals(key, allowanceList.get(j).getDetailName().toString())) {
                                    value = allowanceList.get(j).getDetailValue();
                                    break;
                                }
                            }
                        }
                        ...
                        if (bonusList != null && !bonusList.isEmpty()) {
                            for (int j = 0; j < bonusList.size(); j++) {
                                if (StringUtils.equals(key, bonusList.get(j).getDetailName().toString())) {
                                    value = bonusList.get(j).getDetailValue();
                                    break;
                                }
                            }
                        }
