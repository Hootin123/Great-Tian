...
//        return true;
        CompanyMembersBean sysUserBean = SessionUtils.getUser(request);
        if (sysUserBean != null)
            return true;
        else {
            /**非法登录，跳转到登录页面*/
//            response.sendRedirect(request.getContextPath() + "/loginInit.htm");
            response.getWriter().write("<script>top.location.href=\"/loginInit.htm\"</script>");
