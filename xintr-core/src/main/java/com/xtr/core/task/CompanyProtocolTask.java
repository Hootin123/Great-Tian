 ...
 */
@Service("companyProtocolTask")
public class CompanyProtocolTask extends BaseTask {
...
//                    LOGGER.info("企业协议状态自动变更任务【" + currentTime + "】,到期时间与当前时间相差" + differTime + "天");
                    if (differTime < 0) {//如果当前时间超过了到期时间,则更改状态为合约到期
                        ...
                        if (companyProtocolBean.getProtocolContractType() != null && companyProtocolBean.getProtocolCurrentStatus() != null
                            ...
                            try {
                                ...
                                if (!StringUtils.isStrNull(companyProtocolBean.getCompanyCorporationPhone())) {
                                    companysBean.setCompanyCorporationPhone(companyProtocolBean.getCompanyCorporationPhone());
                                }
        ...
        if (modifyStateListAlready != null && modifyStateListAlready.size() > 0) {
            for (CompanyProtocolsBean paramBean : modifyStateListAlready) {
                ...
                //已到期的订单还是更改发工资的状态
                if(paramBean.getProtocolCurrentStatus().intValue()==CompanyProtocolConstants.PROTOCOL_CURRENTSTATE_ALREADY){
                    //获取要修改的企业协议信息,为了后面修改发工资等状态
                    CompanyProtocolsDto protocolsDto=companyProtocolsService.selectProtocolAndCompanyById(paramBean.getProtocolId());
                    if(protocolsDto!=null && protocolsDto.getProtocolContractType()!=null && protocolsDto.getProtocolContractType().intValue()==CompanyProtocolConstants.PROTOCOL_TYPE_DJ){
                        companyProtocolsService.updateSalaryState( protocolsDto.getProtocolCompanyId(), protocolsDto.getProtocolContractType(),0);
                    }
