...
$(function(){
	...
	$(".check span").click(function(){
		location.href=BASE_PATH+"/h5walletNew/toH5walletAgreePage.htm";
	});
	...
	$(".reg").on('click',function(){
		...
		testCode();
		testPhoneCode();
		if($(".check input").is(':checked')){//点击同意协议才能注册
			if(fcode & faccount & fpwd & fname &fphonecode){
				$.ajax({
					type: 'post',
					url:BASE_PATH+ '/h5walletNew/h5Register.htm',
	...
	$('#phonecode p').on('click',function(){
		...
		if(getCode && faccount){
			$.ajax({
				type: 'post',
				url:BASE_PATH+ '/h5walletNew/sendPhoneMsgByH5.htm',
    ...
    $("#sureBtn").on("click",function(){
		...
		window.location.href = BASE_PATH+"/h5walletNew/toH5walletApprovePage.htm?companyId="+companyId;
	});
});
