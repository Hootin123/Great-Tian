...
$(function(){
	...
	$(".check span").click(function(){
		location.href=BASE_PATH+"/scanCode/toH5ScanAgreePage.htm"
	})
	...
	$(".reg").on('click',function(){
		...
		if(fcode & faccount & fpwd & fname &fphonecode){
			$.ajax({
				type: 'post',
				url: BASE_PATH+'/scanCode/h5ScanRegister.htm',
				...
				success: function (data) {
					...
					if (data.success) {
						...
						//跳转到立即认证的页面
						window.location.href = BASE_PATH+"/scanCode/toAuthenticationPage.htm?companyId="+companyId;
					} else {
	...
	$(".login").on('click',function(){
		window.location.href= BASE_PATH+'/scanCode/toH5ScanLoginPage.htm';
	});
	...
	$('#phonecode p').on('click',function(){
		...
		if(getCode && faccount){
			$.ajax({
				type: 'post',
				url: BASE_PATH+'/scanCode/sendPhoneMsgByScanCode.htm',
...
function changeCodeImg(){
	$("#imageCode").attr("src",'generateScanH5ValidatorImages.htm?time='+new Date().getTime());
}
