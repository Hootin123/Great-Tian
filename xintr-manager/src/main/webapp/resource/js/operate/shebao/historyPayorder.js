...
function queryHistoryBills(){
    $.ajax({
       url:BASE_PATH+'/shebaoManager/queryAllOrderBills.htm',
       type:'post',
       dataType:'json',
       ...
       success:function(data){
         if(data.success){
           ...
           $.each(list,function (i,data) {
              ...
              html+="<p><span>"+data.companyName+"-"+"</span><span>"+data.joinCityName+""+timeDateTostr(data.orderDate)+"</span>账单</p>";
              ...
              html+="</td>";
              html+=" <td rowspan='2' class='showTd1'><a  onclick=orderDetail("+data.id+","+data.companyId+")>账单详情</a><br>";
               //判断该账单是否是异常账单
...
//跳转到账单详情页面
function orderDetail(orderId,companyId){
    var path = BASE_PATH+"/shebaoManager/toOrderDetail.htm?orderId="+orderId+"&companyId="+companyId;
...
function showAddtion(id){
    var path = BASE_PATH+"/shebaoManager/addtion_detail.htm?supplementCompanyOrderId="+id;
    var index = layer.open({
        type: 2,
        title: false,
        maxmin: false,
...
function queryErrorBill(orderId){
    var path = BASE_PATH+"/shebaoManager/errorBillDetail.htm?orderId="+orderId
    var index = layer.open({
        type: 2,
        title: false,
        maxmin: false,
